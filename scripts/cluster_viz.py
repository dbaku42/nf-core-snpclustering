...
def _normalise_id_column(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "IID" in cols_upper:
        ...
        if iid_numeric:
            # Caso C: colonna "IID" è in realtà FID; il vero IID è la colonna successiva
            df = df.drop(columns=[iid_col])
            df = df.rename(columns={df.columns[0]: "sample_id"})
        else:
            # Caso B: IID contiene già gli ID campione
...
def compute_tsne(X: np.ndarray, perplexity: float, max_iter: int) -> np.ndarray:
    return TSNE(
        ...
        max_iter=max_iter, learning_rate="auto",
    ).fit_transform(X)
...
def main() -> None:
    ...
    umap_df = pd.DataFrame({
        "sample_id": common.values, "x": umap_coords[:, 0],
        "y": umap_coords[:, 1], "cluster": y,
    })
    ...
    tsne_df = pd.DataFrame({
        "sample_id": common.values, "x": tsne_coords[:, 0],
        "y": tsne_coords[:, 1], "cluster": y,
    })
    ...
    # PCA (prime due colonne PC da pca_scores)
